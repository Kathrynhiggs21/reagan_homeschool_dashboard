...
- [ ] BLOCKED on Google Drive API credentials — Drive orphan/dupe cleanup: nightly job to detect empty folders and trash them (not delete). Same blocker as above. Implementation will live in the same `driveFolderDedupeJob.ts` module.
...
- [ ] BLOCKED on Google Drive API credentials — Ohio curriculum standards reference file → auto-push into Curriculum and Resources folder on schedule. Same Drive-worker blocker. The reference text itself can be authored as a Markdown doc inside `references/` and enqueued onto `drivePushQueue` today; the actual push to Drive blocks on credentials.
- [ ] BLOCKED on Google Drive API credentials — Drive sub-folder dedupe job (post-push). Same blocker as the items above; this is the same job described under 'Nightly Drive-side folder dedupe'.
...
- [ ] Skills: update `reagan-homeschool-grading` SKILL.md to document 6th grade grading expectations — the skill file is in user-managed Manus skills, not in this repo. Logged for user-side update.
...
- [ ] BLOCKED on Google Drive API credentials — Hash-based skip vs Drive folder: during drive-push drain, before re-uploading a file, list the canonical-target folder's children and skip the upload if any child's content hash already matches. Currently no drive-push worker exists in the codebase — the `drivePushQueue` rows are enqueued but there is no drainer that actually pushes to Drive. Building the worker requires a Google Drive OAuth token or service-account JSON, which has not been provided. Once credentials are available: build `server/_lib/drivePushWorker.ts` that drains pending rows, fetches the bytes from S3 via `storageGet`, lists the target Drive folder's children via `drive.files.list({ q: "'parentId' in parents" })`, computes SHA-256 over each child's bytes (or trusts the existing `md5Checksum` field on Drive metadata), and skips/uploads accordingly. Outcome persisted on queue row (`status: 'pushed'` or `'skipped'`, `errorMessage` set on dedup hit).
- [ ] BLOCKED on Google Drive API credentials — Nightly Drive-side folder dedupe: scheduled job that lists each canonical parent's children, groups by normalized name + content hash, picks a survivor (oldest createdTime), moves children of dupes into the survivor, then trashes the empty dupes. Should never touch the 9 pinned top-level folders. Same blocker as above — no Drive API client exists yet. Once credentials are available, scaffold `server/_lib/driveFolderDedupeJob.ts` and register it as a scheduled task via the heartbeat scheduler.
