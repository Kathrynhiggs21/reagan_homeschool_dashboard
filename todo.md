...
- [ ] Mom + Grandma can edit EVERY field, EVERY block, EVERY day in the AI Agenda Editor — title, subject, start time, duration, body, materials, links, printables — with NO approval gate
  ...
  - [ ] **Open**: Materials / links / printables editing — these are NOT scheduleBlocks columns; they live on attached entities (`adventures.materials`, `curriculumResources`, `dailyPrintables`). Split out as its own bullet:
- [ ] Mom + Grandma can edit a block's attached materials, links, and printables (adventures.materials list, curriculumResources rows, dailyPrintables rows) from a single "attached resources" panel inside the AgendaEditor block row — no approval gate, defense-in-depth via familyAdminProcedure. Inventory first: confirm which tRPC procedures already exist for each table before building UI.
- [ ] **AI Agenda Editor accepts free-form prompts → diff Mom can accept/reject per block.** Two-part: server slice DONE 2026-05-17 — UI slice still open.
  ...
  - [ ] UI slice (NOT YET): Wire AgendaEditor.tsx to `plans.aiPropose` — free-form prompt input, per-block diff cards (one per `keep`/`modify`/`remove`/`add` decision), per-card Accept/Reject toggle, bottom "Apply N changes" button calling `plans.aiApplyProposal` with only the accepted decisions. Empty-state for no-plan dates + warning surface. Toast on success summarizing `+added ~modified -removed`.
  - [ ] UI vitest: AgendaEditor free-form prompt flow — prompt entry mounted, diff cards render per decision kind, accept/reject toggles update state, only accepted decisions reach the apply mutation.
...
- [ ] **Vitest: free-form prompt → diff → commit (server done; integration + UI still open).**
  ...
  - [ ] Real-DB integration vitest for `plans.aiApplyProposal`: insert a plan + 3 blocks, build a decisions array with one `remove` + one `modify` + one `add` (and a `keep` that should NOT be touched), call the apply path, assert exact ending DB state — not the un-accepted decisions, no orphaned sortOrder gaps, audit row written.
  - [ ] Atomicity / failure-mode test: when one of the modify/add operations fails, prove either (a) we wrap the whole apply in a single DB transaction and roll back, or (b) we surface a per-decision result with succeeded/failed flags. Currently the implementation is per-decision try/catch with console.warn — acceptable as a starting point but needs to either become transactional OR the contract needs to be explicit about partial-apply semantics.
