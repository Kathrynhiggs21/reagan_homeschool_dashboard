...
- [ ] BUG: PDF link in nightly agenda email returns "AccessDenied / d36hbw14aib5lz.cloudfront.net" — the email currently embeds a presigned CloudFront URL that has expired by the time the recipient opens the inbox. **Fix per user 2026-05-30:** drop the URL entirely, attach the PDF bytes directly to the email as a MIME attachment. Mailer already supports `attachments` — wire `nightlyAgenda.sendNow` (and the cron-agent path) to read the PDF bytes from storage and pass them through.
...
- [ ] BUG: Route `/calendars` returns 404 — the link/redirect points at a route that doesn't exist in `App.tsx`. Either add the calendar page or redirect `/calendars` to whatever the canonical calendar route is.
- [ ] BUG: On the homepage agenda, tapping an assignment block currently opens the app homepage or a generic subject landing page (e.g., generic Math page) instead of the actual assignment's worksheet/printable. Fix the per-block click handler in the homepage agenda card so it deep-links directly to the assignment view (or the worksheet PDF if that's all there is). Should resolve the same complaint for printables that share the agenda block opener.
...
- [ ] FEATURE: Surface the day's planned blocks (the agenda) as a calendar layer on the Schedule page so the agenda for the day appears as calendar entries alongside iCal feeds and Google Calendar overlay. Day view should show blocks as time-bracketed events; Week view should show them as block-density dots / compact list; the GoogleCalendarEmbed at the bottom should also receive the day's planned block events as overlay items. Server emits a stable iCal feed `/api/agenda.ics` (day-scoped or week-scoped) so the agenda also lands in Mom's external Google Calendar. Optional second pass: persist day's plan as ICS for offline / phone-calendar import.
- [ ] BUG follow-up to email work: Resend's free tier silently drops every recipient that isn't `spear.cpt@gmail.com` until a domain is verified at resend.com/domains. `MAIL_ALLOWED_RECIPIENTS` set to `spear.cpt@gmail.com` so we don't waste sends. Add a Gmail-SMTP fallback (already-set `GMAIL_APP_PASSWORD` + `GMAIL_SMTP_USER`) for any recipient Resend rejects, until domain is verified. Once verified, drop the allow-list.
