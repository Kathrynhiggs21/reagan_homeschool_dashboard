          ...
          {blocks.map((b: any) => (
            <li key={b.id} className="flex flex-col gap-1 p-2 rounded-lg bg-white/70 dark:bg-amber-950/30 border border-amber-200">
              <div className="flex items-center gap-3">
                ...
                {/* v2.86 (2026-05-21) — Removed inline TapEditPopover on /schedule.
                    Mom asked to keep /schedule calendar-only with a single
                    editor surface (the AgendaEditor reached via tapping the
                    block title above). The tap-edit popover still lives on
                    /today for the day Reagan is actually living. */}
            ...
            {blocks.map((b: any) => (
              <li key={b.id} className="flex flex-col gap-1 p-2 rounded-lg bg-amber-50 border border-amber-200">
                <div className="flex items-center gap-3">
                  ...
                  {/* v2.86 (2026-05-21) — Removed inline TapEditPopover from
                      DayPreview too (single-editor rule). Adults open
                      AgendaEditor for any non-today edit. */}
